...
//my original 64 ,ac = 2e-4 , c = 2e-3
public class DDPGAgent_DoublePER extends DDPGAgent {
    ...
    private Random t1;                   // Random generator for target 1 noise
    private Random t2;                   // Random generator for target 2 noise
    private float Nt1;                   // noise for target network 1
    private float Nt2;                   // noise for target network 2
    ...
    private FeedForwardNetwork critic_target2; //critic target2 network
     ...
     */
    public DDPGAgent_DoublePER(int state_size, int action_size){
		super();
       ...
       // t2.setSeed(456);
        ...
        Nt1 = t1.nextFloat()+t1.nextInt(10);
        Nt2 = t2.nextFloat()+t1.nextInt(10);
        writeToMyLog ("Random numbers ..."+Nt1+" , "+Nt2+"\n");
        ...
        critic_target2= configCritic(state_size, action_size, critic_units[0] , critic_units[1], LR_CRITIC);
     ...
     * @return
     */
    public float[] get_TD_error2(float[][] Q_targets , float[][] Q_targets2 ,float[][] Q_expected,int exps_size){
        float[] td_error = new float[exps_size];// bec Q value is 1 for each experiance
        for (int i = 0; i < td_error.length; i++) {
            float td_error1 = Math.abs(Q_targets[i][0] - Q_expected[i][0]);
            float td_error2 = Math.abs(Q_targets2[i][0] - Q_expected[i][0]);
            td_error[i] = td_error1 + td_error2;
        }
        return td_error;
    }
    ...
    public void learn2(Experiance[] exps , double gamma, int t){
        ...
        float[][] Q_target_next2 = new float[exps.length][1];
        ...
        for(int i = 0 ; i<exps.length ;i++){
            ...
            Q_target_next2[i] = critic_target2.predict(next_states_actions);//array to keep track of Q_targets size is 1

            //writeToMyLog ("Agent learn: Q_target_next... "+printFloat(Q_target_next)+"\n");
            // Compute Q targets for current states (y_i)
            //for(int j = 0 ; j<Q_target_next.length;j++){
            float target = rewards[i] + (GAMMA*(Q_target_next[i][0]+Nt1)*(1-dones[i]));
            Q_target_next[i] = new float[]{target};
            float target2 = rewards[i] + (GAMMA*(Q_target_next2[i][0]+Nt2)*(1-dones[i]));
            Q_target_next2[i] = new float[]{target2};
                //writeToMyLog ("Agent learn: Q_target_next "+i+" : "+Q_target_next[j] +"\n");
           // }
            writeToMyLog ("Agent learn: Q_target_next ... "+printFloat(Q_target_next[i])+"\n");
            writeToMyLog ("Agent learn: Q_target_next2 ... "+printFloat(Q_target_next2[i])+"\n");
        ...
        //Compute TD-error δj = |y1-Q1| + |y2-Q2|
        float[] TD_errors = get_TD_error2(Q_target_next,Q_target_next2,Q_expected_next,exps.length);
        //Update transition priority pj
        memory.update_priorities2_double(TD_errors,rewards,exps.length);
       // memory.update_priorities2(TD_errors,exps.length);
        float[] error1 = weighted_mse_loss2(Q_expected_next, Q_target_next,weights);
        float[] error2 = weighted_mse_loss2(Q_expected_next, Q_target_next2,weights);
        float[] error = min_error(error1,error2);

        writeToMyLog("Agent learn: error1 ... " + printFloat(error1) + "\n");
        writeToMyLog("Agent learn: error2 ... " + printFloat(error2) + "\n");
        writeToMyLog("Agent learn: Min error ... " + printFloat(error) + "\n");
        ...
        if(Arrays.equals(error,error1)){
            writeToMyLog ("Agent learn: critic softUpdate with target 1 ...\n");
            critic_target = softUpdate(critic_local,critic_target);
        }else{
            writeToMyLog ("Agent learn: critic softUpdate with target 2 ...\n");
            critic_target = softUpdate(critic_local,critic_target2);
        }
    ...
    public float[] min_error(float[] err1 , float[] err2){

        return err1[0]<err2[0]? err1:err2;
    }
